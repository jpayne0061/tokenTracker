...
export class AccountService2 {
  ...
  updateRole(role: Role) {
    if (role.id) {
      return this.accountEndpoint.getUpdateRoleEndpoint(role, role.id).pipe(
        tap(data => this.onRolesChanged([role], AccountService2.roleModifiedOperation)));
    } else {
  ...
  newRole(role: Role) {
    return this.accountEndpoint.getNewRoleEndpoint<Role>(role).pipe<Role>(
      tap(data => this.onRolesChanged([role], AccountService2.roleAddedOperation)));
  }
  ...
  deleteRole(roleOrRoleId: string | Role): Observable<Role> {
    ...
    if (typeof roleOrRoleId === 'string' || roleOrRoleId instanceof String) {
      return this.accountEndpoint.getDeleteRoleEndpoint<Role>(<string>roleOrRoleId).pipe<Role>(
        tap(data => this.onRolesChanged([data], AccountService2.roleDeletedOperation)));
    } else {
  ...
  onRolesUserCountChanged(roles: Role[] | string[]) {
    return this.onRolesChanged(roles, AccountService2.roleModifiedOperation);
  }
